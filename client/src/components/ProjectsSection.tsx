...
const projects = [
  {
    ...
    description: "A fully custom, multi-page website built for an authentic Mexican taqueria in Queens, NY. Features a digital menu, photo gallery, Google Maps integration, and a mobile-first responsive design that drives real foot traffic and customer engagement.",
  ...
  },
  {
    ...
    description: "A full-stack web platform built exclusively for Brooklyn College students — a professor rating and mentorship discovery system. Features dynamic search, student reviews, faculty profiles, and a real-time recommendation engine built with React and TypeScript.",
...
function ProjectCard({ project, index }: { project: typeof projects[0]; index: number }) {
  ...
  return (
    ...
    >
      ...
      >
        <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: "clamp(0.78rem, 1.5vw, 0.88rem)", color: "rgba(240,238,232,0.72)", lineHeight: 1.7 }}>
          {project.description}
        </p>
        <div className="flex flex-wrap gap-2 mt-3">
          {project.stack.map((tech) => (
            <span
              key={tech}
              style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "0.55rem", letterSpacing: "0.08em", padding: "3px 8px", border: "1px solid rgba(240,238,232,0.2)", color: "rgba(240,238,232,0.55)", textTransform: "uppercase" }}
            >
              {tech}
            </span>
          ))}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 flex items-end justify-between">
        <div>
          <p style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "0.55rem", letterSpacing: "0.12em", color: "rgba(240,238,232,0.4)", textTransform: "uppercase", marginBottom: "4px" }}>
            {project.category}
          </p>
          <h3 style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: "clamp(1.1rem, 2.5vw, 1.75rem)", color: "#F0EEE8", lineHeight: 1.1, letterSpacing: "-0.02em" }}>
            {project.name}
          </h3>
        </div>
        <div className="flex items-center gap-2">
          {project.live && (
            <motion.a
              ...
              style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "0.55rem", letterSpacing: "0.1em", textTransform: "uppercase", padding: "5px 10px", background: "#00C8E0", color: "#0D0D0D", fontWeight: 600 }}
          ...
          <motion.a
            ...
            transition={{ duration: 0.25 }}
            onClick={(e) => e.stopPropagation()}
            className="w-9 h-9 flex items-center justify-center border border-[#F0EEE8]/30 text-[#F0EEE8] hover:bg-[#F0EEE8] hover:text-[#0D0D0D] transition-colors duration-200"
          >
...
export default function ProjectsSection() {
  ...
  return (
    <section id="projects" style={{ background: "#F5F3EE", color: "#0D0D0D" }}>
      ...
      {/* Heading */}
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 pt-14 pb-8" ref={ref}>
        <motion.h2
          initial={{ opacity: 0, y: 30 }}
          animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.8 }}
          style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: "clamp(2rem, 5vw, 4.5rem)", lineHeight: 1, letterSpacing: "-0.04em", color: "#0D0D0D" }}
        >
        ...
        <motion.p
          ...
          style={{ fontFamily: "DM Sans, sans-serif", fontSize: "clamp(0.9rem, 2vw, 1rem)", color: "rgba(13,13,13,0.5)", marginTop: "1rem", maxWidth: "480px" }}
        >
      ...
      {/* View all CTA */}
      <div className="max-w-[1440px] mx-auto px-6 lg:px-12 py-8 flex justify-end">
        <a
          ...
          className="inline-flex items-center gap-2 px-5 py-2.5 border border-[#0D0D0D] text-[#0D0D0D] hover:bg-[#0D0D0D] hover:text-[#F5F3EE] transition-colors duration-200"
        >
